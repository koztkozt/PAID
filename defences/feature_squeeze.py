...
def attack_detection(
    ...
    for i, sample in enumerate(test_data_loader):
        ...
        squeeze_image = reduce_bit(sample[0], 4)
        ...
        plot_fig = False
        if attack_name == "fgsm":
            diff, pred_angle, pred_angle_attack, plt_, _, perturbed_image = fgsm_attack_test(
                stage1, sample[0], target, device, image_size, plot_fig
            )
        elif attack_name == "opt":
            diff, pred_angle, pred_angle_attack, plt_, perturbed_image = optimized_attack_test(
                stage1, sample[0], target, device, image_size, plot_fig
            )
        elif attack_name == "opt_uni":
            ...
            diff, pred_angle, pred_angle_attack, plt_, perturbed_image = advGAN_uni_test(
                stage1, sample[0], device, noise, image_size, plot_fig
            )

        squeeze_perturbed_image = reduce_bit(perturbed_image, 4)
        ...
        print(pred_angle[0], pred_angle_squeezed[0])
...
def defences(config):
    ...
    # attack_names = ["fgsm", "opt", "opt_uni", "advGAN", "advGAN_uni"]
    attack_names = ["advGAN", "opt"]
    ...
    print(df)
    df.to_csv("./results/" + dataset_name + "_" + sys.argv[2] + ".csv")
