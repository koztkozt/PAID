...
from os import path
import time
from imageio import imread  # from scipy.misc import imread, imresize,imsave
...
def calc_curvature(v_ego, angle_steers, angle_offset=0):
    ...
    curvature = angle_steers_rad / (steer_ratio * wheel_base * (1.0 + slip_fator * v_ego**2))
    return curvature
...
def calc_lookahead_offset(v_ego, angle_steers, d_lookahead, angle_offset=0):
    ...
    y_actual = d_lookahead * np.tan(np.arcsin(np.clip(d_lookahead * curvature, -0.999, 0.999)) / 2.0)
