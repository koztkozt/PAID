...
def draw_pt(
    ...
    if row >= 0 and row < img.shape[0] and col >= 0 and col \
        < img.shape[1]:
        img[int(row) - sz:int(row) + sz, int(col) - sz:int(col) + sz] = \
            color
...
def calc_curvature(v_ego, angle_steers, angle_offset=0):
    ...
    curvature = angle_steers_rad / (steer_ratio * wheel_base * (1.
                                    + slip_fator * v_ego ** 2))
    return curvature


def calc_lookahead_offset(
    v_ego,
    angle_steers,
    d_lookahead,
    angle_offset=0,
  ...
  # clip is to avoid arcsin NaNs due to too sharp turns
    ...
    y_actual = d_lookahead * np.tan(np.arcsin(np.clip(d_lookahead
                                    * curvature, -0.999, 0.999)) / 2.)
...
def draw_draw(img, y_pred, y_true=None):
