...
def generate_image(X, X_adv, noise, y_pred, y_adv, image_size):
    ...
    print("min", np.amin(X_adv))
    print("max", np.amax(X_adv))
    plt.imshow(X_adv)
...
def fgsm_attack_test(model, image, target, device, image_size, plot_fig, epsilon=0.01):
    # image = image.unsqueeze(0)
    image = image.type(torch.FloatTensor)
    image = image.to(device)
    diff, perturbed_image, steer, adv_output, noise = fgsm_attack(model, image, target, device, image_size)
    if plot_fig:
        plt = generate_image(
            ...
            steer.detach().cpu().numpy()[0][0],
            adv_output.detach().cpu().numpy()[0][0],
            image_size,
        )
    else:
        plt = 0
...
def optimized_attack_test(model, image, target, device, image_size, plot_fig):
    image = image.type(torch.FloatTensor)
    image = image.to(device)
    perturbed_image, noise, steer, adv_output = optimized_attack(model, target, image, device)
    diff = abs(adv_output.detach().cpu().numpy() - steer.detach().cpu().numpy())
...
def advGAN_test(model, image, advGAN_generator, device, image_size, plot_fig):
    image = image.type(torch.FloatTensor)
    image = image.to(device)
    steer = model(image)
