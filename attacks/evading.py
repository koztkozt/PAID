...
def attacks(config):
    ...
    Y = pd.read_csv(dirparent + "/" + data_path +"Y_all.csv")
    # change to all evade
    Y["angle_new"] = Y["angle_convert_org"] + 0.3
    Y["Anomaly"] = 1
...
def fgsm_ex(test_dataset, stage1, stage2, dataset_name, attack_name, image_nc, target, device, num_sample, image_size):
    ...
    for i, sample in enumerate(test_dataset):
        ...
        diff, pred_angle, pred_angle_attack, plt_, noise, _ = fgsm_attack_test(
            stage1, sample[0], target, device, image_size, False
        )
...
def opt_uni_ex(
    ...
    for i, sample in enumerate(test_dataset):
        ...
        diff, pred_angle, pred_angle_attack, plt_, _ = optimized_uni_test(
            stage1, sample[0], device, noise, image_size, False
        )
        diff_total.extend(diff)
        # print(pred_angle, pred_angle_attack)
        if abs(diff) > abs(target):
            opt_uni_success += 1
...
def advGAN_ex(
    ...
    for i, sample in enumerate(test_dataset):
        ...
        diff, pred_angle, pred_angle_advGAN, plt_, noise, _ = advGAN_test(
            stage1, sample[0], advGAN_generator, device, image_size, False
        )
        diff_total.extend(diff)
...
def save_attack_success(dataset_name,attack_name, ast):
    ...
    with open("results/"+dataset_name+"_"+attack_name + "_evade_success.txt", "w") as f:
        f.write(str(ast))
